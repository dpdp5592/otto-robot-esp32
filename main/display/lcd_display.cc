...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    auto apply_scale = [this](const lv_img_dsc_t* dsc) {
        if (!dsc || dsc->header.w == 0 || dsc->header.h == 0) {
            return;
        }
        const int padding = 8;
        int max_w = width_ > 2 * padding ? width_ - 2 * padding : width_;
        int max_h = height_ > 2 * padding ? height_ - 2 * padding : height_;
        int scale_w = 256 * max_w / dsc->header.w;
        int scale_h = 256 * max_h / dsc->header.h;
    ...
    if (image->IsGif()) {
        ...
        if (gif_controller_->IsLoaded()) {
            ...
            lv_image_set_src(emoji_image_, gif_controller_->image_dsc());
            apply_scale(gif_controller_->image_dsc());
            gif_controller_->Start();
