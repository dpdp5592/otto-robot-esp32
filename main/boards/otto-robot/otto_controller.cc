...
private:
    ...
    void LoadHomeFromNVS() {
        Settings settings("otto_home", false);

        int left_leg = settings.GetInt("left_leg", 90);
        int right_leg = settings.GetInt("right_leg", 90);
        int left_foot = settings.GetInt("left_foot", 90);
        int right_foot = settings.GetInt("right_foot", 90);
        int left_hand = settings.GetInt("left_hand", Otto::kHandHomePosition);
        int right_hand = settings.GetInt("right_hand", 180 - Otto::kHandHomePosition);

        ESP_LOGI(TAG,
                 "从NVS加载初始姿态: 左腿=%d, 右腿=%d, 左脚=%d, 右脚=%d, 左手=%d, 右手=%d",
                 left_leg, right_leg, left_foot, right_foot, left_hand, right_hand);

        otto_.SetHomeAngles(left_leg, right_leg, left_foot, right_foot, left_hand, right_hand);
    }
...
public:
    ...
    void RegisterMcpTools() {
        ...
        mcp_server.AddTool("self.otto.save_home",
                           "保存当前舵机姿态为初始姿态(home)，后续步态以此为中心",
                           PropertyList(), [this](const PropertyList& properties) -> ReturnValue {
                               int positions[SERVO_COUNT] = {0};
                               otto_.GetEffectivePositions(positions);

                               Settings settings("otto_home", true);
                               settings.SetInt("left_leg", positions[LEFT_LEG]);
                               settings.SetInt("right_leg", positions[RIGHT_LEG]);
                               settings.SetInt("left_foot", positions[LEFT_FOOT]);
                               settings.SetInt("right_foot", positions[RIGHT_FOOT]);
                               settings.SetInt("left_hand", positions[LEFT_HAND]);
                               settings.SetInt("right_hand", positions[RIGHT_HAND]);

                               otto_.SetHomeAngles(positions[LEFT_LEG], positions[RIGHT_LEG],
                                                   positions[LEFT_FOOT], positions[RIGHT_FOOT],
                                                   positions[LEFT_HAND], positions[RIGHT_HAND]);

                               std::string result =
                                   "{\"left_leg\":" + std::to_string(positions[LEFT_LEG]) +
                                   ",\"right_leg\":" + std::to_string(positions[RIGHT_LEG]) +
                                   ",\"left_foot\":" + std::to_string(positions[LEFT_FOOT]) +
                                   ",\"right_foot\":" + std::to_string(positions[RIGHT_FOOT]) +
                                   ",\"left_hand\":" + std::to_string(positions[LEFT_HAND]) +
                                   ",\"right_hand\":" + std::to_string(positions[RIGHT_HAND]) + "}";

                               ESP_LOGI(TAG, "保存初始姿态: %s", result.c_str());
                               return result;
                           });

        mcp_server.AddTool(
            "self.otto.pose",
            "设置当前姿态用于校准初始姿态，角度范围0-180。未指定的舵机保持当前角度。",
            PropertyList({Property("left_leg", kPropertyTypeInteger, -1, -1, 180),
                          Property("right_leg", kPropertyTypeInteger, -1, -1, 180),
                          Property("left_foot", kPropertyTypeInteger, -1, -1, 180),
                          Property("right_foot", kPropertyTypeInteger, -1, -1, 180),
                          Property("left_hand", kPropertyTypeInteger, -1, -1, 180),
                          Property("right_hand", kPropertyTypeInteger, -1, -1, 180),
                          Property("speed", kPropertyTypeInteger, 800, 100, 3000)}),
            [this](const PropertyList& properties) -> ReturnValue {
                int targets[SERVO_COUNT] = {0};
                otto_.GetServoPositions(targets);

                int home_angles[SERVO_COUNT] = {0};
                otto_.GetHomeAngles(home_angles);
                int default_angles[SERVO_COUNT] = {90, 90, 90, 90, Otto::kHandHomePosition,
                                                   180 - Otto::kHandHomePosition};
                int home_offsets[SERVO_COUNT] = {0};
                for (int i = 0; i < SERVO_COUNT; i++) {
                    home_offsets[i] = home_angles[i] - default_angles[i];
                }

                int left_leg = properties["left_leg"].value<int>();
                int right_leg = properties["right_leg"].value<int>();
                int left_foot = properties["left_foot"].value<int>();
                int right_foot = properties["right_foot"].value<int>();
                int left_hand = properties["left_hand"].value<int>();
                int right_hand = properties["right_hand"].value<int>();
                int speed = properties["speed"].value<int>();

                auto apply_target = [&](int index, int value) {
                    if (value < 0) {
                        return;
                    }
                    int cmd = value - home_offsets[index];
                    if (cmd < 0) cmd = 0;
                    if (cmd > 180) cmd = 180;
                    targets[index] = cmd;
                };

                apply_target(LEFT_LEG, left_leg);
                apply_target(RIGHT_LEG, right_leg);
                apply_target(LEFT_FOOT, left_foot);
                apply_target(RIGHT_FOOT, right_foot);

                if (left_hand >= 0 || right_hand >= 0) {
                    if (!has_hands_) {
                        return "错误：机器人没有配置手部舵机";
                    }
                    apply_target(LEFT_HAND, left_hand);
                    apply_target(RIGHT_HAND, right_hand);
                }

                char sequence_json[256];
                snprintf(sequence_json, sizeof(sequence_json),
                         "{\"a\":[{\"s\":{\"ll\":%d,\"rl\":%d,\"lf\":%d,\"rf\":%d,\"lh\":%d,"
                         "\"rh\":%d},\"v\":%d}]}",
                         targets[LEFT_LEG], targets[RIGHT_LEG], targets[LEFT_FOOT],
                         targets[RIGHT_FOOT], targets[LEFT_HAND], targets[RIGHT_HAND], speed);

                QueueServoSequence(sequence_json);
                return true;
            });
