...
class Otto {
public:
    static constexpr int kHandHomePosition = 45;
    ...
    void SetTrims(int left_leg, int right_leg, int left_foot, int right_foot, int left_hand = 0,
                  int right_hand = 0);
    void SetHomeAngles(int left_leg, int right_leg, int left_foot, int right_foot,
                       int left_hand = kHandHomePosition,
                       int right_hand = 180 - kHandHomePosition);
    void GetHomeAngles(int out[SERVO_COUNT]) const;
    void GetServoPositions(int out[SERVO_COUNT]) const;
    void GetEffectivePositions(int out[SERVO_COUNT]) const;
...
private:
    ...
    int home_angles_[SERVO_COUNT];
    int home_offset_[SERVO_COUNT];
    ...
    void Execute(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT], int period,
                 double phase_diff[SERVO_COUNT], float steps);
    void ApplyHomeOffsets();
    int DefaultHomeAngle(int index) const;
