...
[[maybe_unused]] static const char* TAG = "OttoMovements";
...
Otto::Otto() {
    ...
    for (int i = 0; i < SERVO_COUNT; i++) {
        servo_pins_[i] = -1;
        servo_trim_[i] = 0;
        home_angles_[i] = DefaultHomeAngle(i);
        home_offset_[i] = 0;
    }
}
...
void Otto::SetHomeAngles(int left_leg, int right_leg, int left_foot, int right_foot, int left_hand,
                         int right_hand) {
    home_angles_[LEFT_LEG] = std::clamp(left_leg, 0, 180);
    home_angles_[RIGHT_LEG] = std::clamp(right_leg, 0, 180);
    home_angles_[LEFT_FOOT] = std::clamp(left_foot, 0, 180);
    home_angles_[RIGHT_FOOT] = std::clamp(right_foot, 0, 180);

    if (has_hands_) {
        home_angles_[LEFT_HAND] = std::clamp(left_hand, 0, 180);
        home_angles_[RIGHT_HAND] = std::clamp(right_hand, 0, 180);
    } else {
        home_angles_[LEFT_HAND] = DefaultHomeAngle(LEFT_HAND);
        home_angles_[RIGHT_HAND] = DefaultHomeAngle(RIGHT_HAND);
    }

    ApplyHomeOffsets();
}

void Otto::GetHomeAngles(int out[SERVO_COUNT]) const {
    for (int i = 0; i < SERVO_COUNT; i++) {
        out[i] = home_angles_[i];
    }
}

void Otto::GetServoPositions(int out[SERVO_COUNT]) const {
    for (int i = 0; i < SERVO_COUNT; i++) {
        if (servo_pins_[i] != -1) {
            out[i] = servo_[i].GetPosition();
        } else {
            out[i] = DefaultHomeAngle(i);
        }
    }
}

void Otto::GetEffectivePositions(int out[SERVO_COUNT]) const {
    for (int i = 0; i < SERVO_COUNT; i++) {
        int base = (servo_pins_[i] != -1) ? servo_[i].GetPosition() : DefaultHomeAngle(i);
        int effective = base + home_offset_[i];
        out[i] = std::clamp(effective, 0, 180);
    }
}

void Otto::ApplyHomeOffsets() {
    for (int i = 0; i < SERVO_COUNT; i++) {
        home_offset_[i] = home_angles_[i] - DefaultHomeAngle(i);
        if (servo_pins_[i] != -1) {
            servo_[i].SetHomeOffset(home_offset_[i]);
        }
    }
}

int Otto::DefaultHomeAngle(int index) const {
    switch (index) {
        case LEFT_HAND:
            return kHandHomePosition;
        case RIGHT_HAND:
            return 180 - kHandHomePosition;
        default:
            return 90;
    }
}
        ...
        for (int i = 0; i < SERVO_COUNT; i++) {
            if (i == LEFT_HAND || i == RIGHT_HAND) {
                if (hands_down) {
                    // 如果需要复位手部，设置为默认值
                    if (i == LEFT_HAND) {
                        homes[i] = kHandHomePosition;
                    } else {                                  // RIGHT_HAND
                        homes[i] = 180 - kHandHomePosition;  // 右手镜像位置
                    }
                } else {
                    // 如果不需要复位手部，保持当前位置
...
void Otto::Jump(float steps, int period) {
    int up[SERVO_COUNT] = {90, 90, 150, 30, kHandHomePosition, 180 - kHandHomePosition};
    MoveServos(period, up);
    int down[SERVO_COUNT] = {90, 90, 90, 90, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::Walk(float steps, int period, int dir, int amount) {
    ...
    int O[SERVO_COUNT] = {0, 0, 5, -5, kHandHomePosition - 90, kHandHomePosition};
...
void Otto::Bend(int steps, int period, int dir) {
    ...
    int bend1[SERVO_COUNT] = {90, 90, 62, 35, kHandHomePosition, 180 - kHandHomePosition};
    int bend2[SERVO_COUNT] = {90, 90, 62, 105, kHandHomePosition, 180 - kHandHomePosition};
    int homes[SERVO_COUNT] = {90, 90, 90, 90, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::ShakeLeg(int steps, int period, int dir) {
    ...
    int shake_leg1[SERVO_COUNT] = {90, 90, 58, 35, kHandHomePosition, 180 - kHandHomePosition};
    int shake_leg2[SERVO_COUNT] = {90, 90, 58, 120, kHandHomePosition, 180 - kHandHomePosition};
    int shake_leg3[SERVO_COUNT] = {90, 90, 58, 60, kHandHomePosition, 180 - kHandHomePosition};
    int homes[SERVO_COUNT] = {90, 90, 90, 90, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::UpDown(float steps, int period, int height) {
    ...
    int O[SERVO_COUNT] = {0, 0, height, -height, kHandHomePosition, 180 - kHandHomePosition};
    double phase_diff[SERVO_COUNT] = {0, 0, DEG2RAD(-90), DEG2RAD(90), 0, 0};
...
void Otto::Swing(float steps, int period, int height) {
    ...
    int A[SERVO_COUNT] = {0, 0, height, height, 0, 0};
    int O[SERVO_COUNT] = {
        0, 0, height / 2, -height / 2, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::Jitter(float steps, int period, int height) {
    ...
    int O[SERVO_COUNT] = {0, 0, 0, 0, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::AscendingTurn(float steps, int period, int height) {
    ...
    int O[SERVO_COUNT] = {
        0, 0, height + 4, -height + 4, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::Moonwalker(float steps, int period, int height, int dir) {
    ...
    int A[SERVO_COUNT] = {0, 0, height, height, 0, 0};
    int O[SERVO_COUNT] = {
        0, 0, height / 2 + 2, -height / 2 - 2, kHandHomePosition, 180 - kHandHomePosition};
    int phi = -dir * 90;
...
void Otto::Crusaito(float steps, int period, int height, int dir) {
    ...
    int O[SERVO_COUNT] = {
        0, 0, height / 2 + 4, -height / 2 - 4, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::Flapping(float steps, int period, int height, int dir) {
    ...
    int O[SERVO_COUNT] = {
        0, 0, height - 10, -height + 10, kHandHomePosition, 180 - kHandHomePosition};
...
void Otto::HandsUp(int period, int dir) {
    ...
    int target[SERVO_COUNT] = {90, 90, 90, 90, kHandHomePosition, 180 - kHandHomePosition};
